...
ADMIN_PKGS=(snapper snap-pac inotify-tools btrfs-assistant zram-generator tpm2-tss)
...
DESKTOP_PKGS=(polkit-gnome network-manager-applet kitty vivaldi vivaldi-ffmpeg-codecs gnome-keyring)
...
sed -i '\#^[^#].*[[:space:]]/[[:space:]]#s|,subvol=/@||' /mnt/etc/fstab
ok "fstab generated"
...
options root=/dev/mapper/cryptroot rw ${KERNEL_PARAMS}
EOF
...
# GNOME Keyring — PAM auto-unlock at login
# cosmic-greeter uses PAM service "login" (set in cosmic-greeter.toml)
